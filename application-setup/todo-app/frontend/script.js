...
async function saveTask() {
  ...
  try {
    ...
    if (editTaskId === null) {
      ...
      editTaskId = null;
      elements.addBtn.innerText = "➕ Add Task";
    }
...
function editTask(id) {
  ...
  elements.addBtn.innerText = "✏ Update Task";

  expandCard();
}

// Make functions available for inline HTML buttons
window.del = del;
...
const categoryIcon = categoryBadge[task.category] || "📌";
...
function updateStats() {
  ...
  elements.pending.textContent = tasks.length - completed;
...
function clearForm() {
  ...
  elements.priority.selectedIndex = 0;
...
function toast(message, type = "success") {
  ...
  elements.toastBox.appendChild(div);
...
function expandCard() {
  ...
  elements.inputCard.classList.remove("collapsed-note");
...
function collapseCard() {
  ...
  elements.inputCard.classList.add("collapsed-note");

  titleInput.placeholder = "Take a note...";
...
function bindEvents() {
  ...
  elements.addBtn.addEventListener("click", saveTask);

  elements.clearBtn.addEventListener("click", clearForm);

  elements.search.addEventListener("input", searchTasks);

  elements.themeBtn.addEventListener("click", toggleTheme);

  elements.inputCard.addEventListener("click", () => {
    expandCard();
  });

  document.addEventListener("click", (e) => {

    if (!elements.inputCard.contains(e.target)) {
      collapseCard();
    }
